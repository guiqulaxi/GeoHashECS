    ...
    public:
        ...
        void nearestKSearch(const point_type &searchPoint, std::size_t K, std::vector<point_type> &ps, std::vector<float> &dis) const
        {
            ...
            else if (K >= _size)
            {
                ...
                for (int i = 0; i != K; ++i)
                    ps.at(i) = nodes.at(i)->_p, dis.at(i) = distanceGeo(nodes.at(i)->_p, searchPoint);
            ...
            while (!searchPath.empty())
            {
                ...
                auto curDis = distanceGeo(curNode->_p, searchPoint);
                if (record.size() < K)
                {
        ...
        void radiusSearch(const point_type &searchPoint, float radius, std::vector<point_type> &ps, std::vector<float> &dis) const
        {
            ...
            while (!searchPath.empty())
            {
                ...
                auto d = distanceGeo(curNode->_p, searchPoint);
                if (d <= radius)
...
#pragma region kdtree2
    ...
    private:
        virtual void buildKdTree(pointset_type ps) override
        {
            ...
            {
                // split feature : Y
                std::sort(ps.begin(), ps.end(), [](const point_type &p1, const point_type &p2)
                          { return p1.y() < p2.y(); });
                sf = SplitFeature::Y;
            }
            else
        ...
        virtual void sortPointSet(SplitFeature sf, pointset_type &lps, pointset_type &rps) override
        {
            ...
            {
                std::sort(lps.begin(), lps.end(), [](const point_type &p1, const point_type &p2)
                          { return p1.x() < p2.x(); });
                std::sort(rps.begin(), rps.end(), [](const point_type &p1, const point_type &p2)
                          { return p1.x() < p2.x(); });
            }
            else
            {
